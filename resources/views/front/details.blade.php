	...
	<section id="Article-container" class="max-w-[1130px] mx-auto flex gap-20 mt-[50px]">
		...
		<div class="side-bar flex flex-col w-[300px] shrink-0 gap-10">
			...
			<div id="More-from-author" class="flex flex-col gap-4">
				...
				@forelse($authornews as $itemnews)
				<a href="{{route('front.details', $itemnews->slug)}}" class="card-from-author">
					<div
						...
						<div class="w-[70px] h-[70px] flex shrink-0 overflow-hidden rounded-2xl">
							<img src="{{Storage::url($itemnews->thumbnail)}}" class="object-cover w-full h-full"
								alt="thumbnail">
						</div>
						<div class="flex flex-col gap-[6px]">
							<p class="line-clamp-2 font-bold">{{$itemnews->name}}</p>
							<p class="text-xs leading-[18px] text-[#A3A6AE]">{{$itemnews->created_at->format('M d, Y')}} - {{$itemnews->category->name}}</p>
						</div>
					</div>
				</a>
				@empty 
					...
					<div
						...
						<div class="thumbnail-container h-[200px] relative rounded-[20px] overflow-hidden">
							<div
								...
								<p class="text-xs leading-[18px] font-bold upperacase">{{$article->category->name}}</p>
							</div>
